...
-- Fix6: Drop via EXECUTE + OID-basiert
DO $body$
DECLARE
  fn_oid oid;
  fn_sig text;
BEGIN
  -- Drop alle Varianten von rpc_get_ambassador_referrals
  FOR fn_oid IN SELECT oid FROM pg_proc WHERE proname = 'rpc_get_ambassador_referrals' LOOP
    ...
    EXECUTE 'DROP FUNCTION IF EXISTS ' || fn_sig || ' CASCADE';
    RAISE NOTICE 'Dropped: %', fn_sig;
  END LOOP;

  -- Drop alle Varianten von rpc_get_all_ambassadors

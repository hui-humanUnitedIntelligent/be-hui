...
// eines genehmigten Impact-Projekts über die Website.
//
// Nutzt die BESTEHENDE HUI-Zahlungsinfrastruktur (gleicher Stripe-
// Account). Die Verbuchung nach erfolgreicher Zahlung läuft über die
// bestehende App-RPC rpc_add_project_direct_support (Migration 141)
// — serverseitig, idempotent und ohne Änderung an App, Webhook oder
// Supabase-Schema (Details: api/impact-project.js).
//
// Ehrlichkeit:
//   - KEINE Erfolgsmeldung an dieser Stelle — Erfolg wird ausschließlich
//     serverseitig nach Rückkehr von Stripe verifiziert und verbucht
//
// Sicherheit:
...
//   - Projekt-Validierung serverseitig: nur status='approved' UND
//     is_completed=false (identischer Schutz wie die App-Edge-Function
//     create-project-support-payment)
//   - Betragsvalidierung: 1,00 € – 5.000 € (identisch zur App),
//     Ganzzent-Rundung
// ══════════════════════════════════════════════════════════════════
...
const MIN_AMOUNT_EUR = 1;    // wie create-project-support-payment (MIN_AMOUNT_CENTS = 100)
const MAX_AMOUNT_EUR = 5000; // wie create-project-support-payment (MAX_AMOUNT_CENTS = 500000)
...
module.exports = async (req, res) => {
  ...
  if (!amountEur || Number.isNaN(amountEur) || amountEur < MIN_AMOUNT_EUR || amountEur > MAX_AMOUNT_EUR) {
    return json(res, 400, { error: `Betrag zwischen ${String(MIN_AMOUNT_EUR).replace('.', ',')} € und ${MAX_AMOUNT_EUR} € wählen.`, code: 'INVALID_AMOUNT' });
  }
  ...
  // Identischer Schutz wie die App: genehmigt UND nicht abgeschlossen
  if (project.status !== 'approved' || project.is_completed) {
    return json(res, 400, { error: 'Dieses Projekt kann aktuell nicht direkt unterstützt werden.', code: 'PROJECT_NOT_SUPPORTED' });
  }
  ...
  params.set('line_items[0][price_data][product_data][name]', `HUI Impact — ${project.project_name}`.slice(0, 120));
  // Eindeutige Projekt-Zuordnung. payment_type bewusst UNTERSCHIEDLICH
  // zur App ('project_direct_support'), damit der App-Webhook
  // Website-Zahlungen nicht doppelt verarbeitet — die Verbuchung
  // übernimmt api/impact-project.js über die App-RPC.
  params.set('metadata[payment_type]', 'website_project_direct_support');
  ...
  params.set('payment_intent_data[description]', `HUI Impact — Unterstützung: ${project.project_name}`.slice(0, 300));

...
function exportFilename() {
  ...
  return `hui-feed-debug-${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}.json`;
}
...
export function resetFeedDiagnostics() {
  const store = initStore();
  if (!store) return null;

  store.startedAt = nowIso();
  store.snapshot = null;
  store.lastSuccessfulRender = null;
  store.loadMoreCalls = [];
  store.observerEvents = [];
  store.anomalies = [];
  store.events = [];
  store.overlayEnabled = isFeedDebugOverlayEnabled();
...
export function attachFeedDebugExport() {
  ...
  store.reset = function resetFeedDebug() {
    return resetFeedDiagnostics();
  };

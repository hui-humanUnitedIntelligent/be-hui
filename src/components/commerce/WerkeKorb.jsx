...
function ImpactZeile({ impactEur }) {
  ...
  return (
    <div style={{
      ...
      {/* Betrag — zentriert, hervorgehoben */}
...
// ════════════════════════════════════════════════════════════════
//  PREISBLOCK — modular, zukunftssicher
// ════════════════════════════════════════════════════════════════
function PreisBlock({ werke, versand, rabatt }) {
  ...
  // Vereinfacht: nur Gesamtbetrag ohne Aufschlüsselung
  ...
  // Aufgeschlüsselt: mehrere Positionen
  const rows = [
    { label: "Werke",   val: werke,   color: C.inkMid, sign: "",  custom: null },
    ...(showVersand ? [{
      label: "Versand",
      val:   versand,
    ...
    ...(showRabatt ? [{
      label: "Rabatt", val: rabatt, color: C.sage, sign: "−", custom: null,
    }] : []),
  ];
    ...
    <div style={{ padding: "4px 0 10px" }}>
      ...
      {rows.map((r, i) => (
        <div key={i} style={{
          ...
          <span style={{ fontSize:13, color:C.muted, fontWeight:400 }}>{r.label}</span>
          <span style={{ fontSize:13, fontWeight:500, color:r.color, fontVariantNumeric:"tabular-nums" }}>
            {r.custom ?? `${r.sign}${fmt(r.val)}`}
          </span>
        </div>
      ))}
    ...
    <>
      ...
      <div
        style={{
          ...
          // paddingBottom: im Footer (inkl. TabBar-Clearance)
          overflow:      "hidden",
        }}
      >
        {/* Handle */}
        ...
        {/* ── Sticky Footer v4.0 ─────────────────────────────────────────────
             Reihenfolge: Deine Auswahl → Impact → Unterstützen-Button
             Safe-Area: env(safe-area-inset-bottom) + TabBar-Clearance (≈56px)
        ...
        {iCount > 0 && phase !== "success" && (
          <div style={{
            ...
            {/* 3. Unterstützen-Button — sticky, immer sichtbar */}
            <button
      ...
      </div>erce/WerkeKorb.jsx — HUI Werkekorb v1.1
// Persönlicher Sammelraum. Kein Warenkorb. Ruhiger menschlicher Raum.
// Designsprache: HUI Design System (hui.design.js + hui.interaction.js)

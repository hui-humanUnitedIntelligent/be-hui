...
const SOCIAL_APPS = [
  ...
  },
  {
    ...
    // TELEGRAM-DUP-URL-FIX (2026-09-13, Michael-Screenshot: Link stand zweimal
    // in der geteilten Nachricht): Telegrams share/url-Dialog stellt den `url`-
    // Parameter SELBST als eigene Zeile + Link-Preview-Karte voran. `text`
    // (buildShareText) enthält den Link aber SCHON eingebettet (intro\n\nurl\n\n
    // footer) -- macht Telegram OHNE Fix ZWEI Kopien des Links draus. Fix: die
    // eingebettete URL aus dem Text entfernen, bevor er an Telegram geht --
    // Telegrams eigener url-Parameter übernimmt den Link + die Vorschau-Karte.
    build: (text, url) => {
      const noUrlText = text.split(url).join("").replace(/\n{3,}/g, "\n\n").trim();
      return `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(noUrlText)}`;
    },
  },
  {

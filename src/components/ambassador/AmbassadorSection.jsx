...
function StatBox({ icon, label, value, accent }) {
  return (
    <div style={{ background: '#F7F5F2', borderRadius: 12, padding: '12px 14px', flex: 1, minWidth: 80 }}>
...
export function AmbassadorBadge({ level, size = 'sm' }) {
  ...
  return (
    <span style={{
      ...
      fontSize: font, fontWeight: 700, color: cfg.color,
      flexShrink: 0,
    }}>
      {cfg.icon} {cfg.label}
    </span>
...
export function AmbassadorCTA({ isAmbassador, isPending, onApply }) {
  if (isAmbassador) return null; // Ambassador-Bereich wird stattdessen gezeigt

...
import { useHomeOptional } from "../home/HomeShell.jsx";
...
import { MeineInhaltePage }    from "../../pages/studio/StudioSubPages.jsx";
...
export default function HuiStudio({
  ...
  const homeCtx = useHomeOptional();
  const switchTab = homeCtx?.switchTab ?? (() => {});
  ...
  const [showMeineInhalte,     setShowMeineInhalte]     = useState(false);
  ...
  // Deep-Link: /studio/:section oder ?section=
  useEffect(() => {
    if (!initialSection) return;
    const openers = {
      support: () => setShowSupport(true),
      tickets: () => setShowMeineTickets(true),
      content: () => setShowMeineInhalte(true),
    };
    openers[initialSection]?.();
  }, [initialSection]);
  ...
  return createPortal(
    <div style={{
      ...
      <div className="studio-scroll" style={{
        ...
        <StudioSection label="Deine kreative Arbeit">
          <StudioRow icon="🎨" label="Werke & Inhalte" onPress={() => setShowMeineInhalte(true)} last />
        </StudioSection>
        <Gap h={20}/>
      ...
      {showMeineInhalte && (
        <MeineInhaltePage
          onBack={() => setShowMeineInhalte(false)}
          userId={profile?.id}
        />
      )}
      {showMeineTickets && (

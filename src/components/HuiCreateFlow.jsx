  ...
  async function publish() {
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage
        .from("media").upload(path, await toSafeUploadBody(file), { contentType: file.type, upsert: false, cacheControl:"public, max-age=31536000, immutable" });
...
export default function HuiCreateFlow({ onClose, onSuccess, initialType = null }) {
  ...
  async function handlePublish(payload) {
    ...
    try {
      ...
      const { error:upErr } = await supabase.storage
        .from(bucket).upload(path, await toSafeUploadBody(file), { contentType:file.type, upsert:false, cacheControl:"public, max-age=31536000, immutable" });
      if (upErr) throw upErr;
  ...
  return (
    <div style={{
      ...
      paddingBottom:"max(0px, max(var(--hui-safe-bottom, 0px), env(safe-area-inset-bottom, 0px), 0px))",
    }}

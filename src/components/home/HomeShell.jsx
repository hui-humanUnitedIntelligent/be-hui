...
export default function HomeShell({ children }) {
  ...
  const { activeSurface, openSurface, closeSurface, forceRecoverWorld } = useWorldSurface();
  ...
  const centralCloseFlow = useCallback((reason = "runtime-close", opts = {}) => {
    const {
      preserveProfile = false,
      preserveChat = false,
      preserveReturnProfile = false,
      recoverWorld = false,
    } = opts || {};
    ...
    if (recoverWorld) forceRecoverWorld(reason);
    cleanupOrbEnvironment({ reason: `central-close-${reason}` });
    if (!preserveReturnProfile) flowStore.clear?.(reason);
  }, [closeOrbWorld, closeSurface, forceRecoverWorld, flowStore]);
  ...
  const switchTab = useCallback((newTab) => {
    ...
    centralCloseFlow(`tab:${newTab}`, { recoverWorld: activeSurface != null });
    _setTab(newTab);
  ...
  }, [_setTab, activeSurface, centralCloseFlow, tab]);

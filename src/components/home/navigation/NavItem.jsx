...
function NavIcon({ k, active }) {
  ...
  if (k === "impact") return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M12 14 Q8 12 7 8 Q10 8 12 11" fill={active ? `${C.teal}30` : "rgba(80,80,80,0.12)"}
        stroke={col} strokeWidth={sw-0.3} strokeLinejoin="round"/>
      <path d="M12 17 Q16 15 17 11 Q14 11 12 14" fill={active ? `${C.teal}22` : "rgba(80,80,80,0.08)"}
        stroke={col} strokeWidth={sw-0.3} strokeLinejoin="round"/>
...
export default function NavItem({ item, isActive, onPress, badge = 0 }) {
  ...
  function handleClick(e) {
    e.stopPropagation();
    console.log("[HUI-NAV] NavItem click:", item?.key);
    if (typeof onPress === "function") {
      onPress(item.key);
    } else {
      console.error("[HUI-NAV] onPress ist keine Funktion!", typeof onPress);
    }
  }

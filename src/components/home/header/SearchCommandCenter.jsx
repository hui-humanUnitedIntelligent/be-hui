...
// Rendert nur auf < 768px — ersetzt den Feed vollständig
// ─────────────────────────────────────────────────────────────
const MOBILE_CATS = [
...
function MobileSearchView({ query, setQuery, onClose, onSelect, currentUser }) {
  const [activeCategory, setActiveCategory] = useState("alle");
  const [localQ, setLocalQ] = useState(query || "");
  const inputRef = useRef(null);
  const debouncedQ = useDebounce(localQ, 280);
  ...
  // Back-swipe / Hardware-Back
  useEffect(() => {
    function onPop(e) { e.preventDefault(); onClose(); }
  ...
  function filterByCategory(cat) {
    ...
    if (cat === "personen")   return results.profiles.filter(r => !r.talent);
    if (cat === "talente")    return results.profiles.filter(r => r.talent);
    if (cat === "werke")      return results.works;
    if (cat === "erlebnisse") return results.experiences;
    if (cat === "beitraege")  return results.momente;
    if (cat === "projekte")   return []; // future
    return [];
  }

  const filtered = filterByCategory(activeCategory);

  // Kategorie-Badge-Zahlen
  function catCount(cat) {
    if (cat === "alle")       return total;
    if (cat === "personen")   return results.profiles.filter(r => !r.talent).length;
    if (cat === "talente")    return results.profiles.filter(r => r.talent).length;
    if (cat === "werke")      return results.works.length;
    if (cat === "erlebnisse") return results.experiences.length;
    if (cat === "beitraege")  return results.momente.length;
    if (cat === "projekte")   return 0;
    return 0;
  }
  ...
  const content = (
    <div style={{
      ...
      overflow: "hidden",
      animation: "msvIn .22s cubic-bezier(.22,1,.36,1) both",
    }}>
      <style>{`
        @keyframes msvIn {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            boxShadow: "0 0 0 3px rgba(14,196,184,0.08)",
            padding: "0 12px",
            height: 42,
          }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" style={{flexShrink:0,opacity:.45}}>
              <circle cx="11" cy="11" r="7" stroke="#0EC4B8" strokeWidth="2"/>
              <path d="M20 20L16.5 16.5" stroke="#0EC4B8" strokeWidth="2" strokeLinecap="round"/>
            </svg>
            <input
              ref={inputRef}
              value={localQ}
            ...
            />
            {localQ && (
              <button onClick={() => setLocalQ("")} style={{
        ...
        {/* Kategorie-Tabs — nur wenn Suchergebnis vorhanden */}
        {showResults && (
          <div style={{
                ...
                >
                  ...
                  {cnt > 0 && (
                    <span style={{
                      ...
                      color: active ? "#0EC4B8" : "rgba(26,53,48,0.4)",
      ...
      <div className="msv-scroll" style={{
        ...
        {!showResults && (
          <div style={{padding: "20px 16px"}}>
            ...
            {["Künstler", "Workshops", "Nachhaltige Projekte", "Musik", "Community"].map((s, i) => (
              <button key={i} onClick={() => setLocalQ(s)} style={{
                display: "flex", alignItems: "center", gap: 10,
                ...
                <span style={{fontSize: 14, fontWeight: 500, color: "#1A3530"}}>{s}</span>
              </button>
            ))}
          </div>
        )}
        ...
        {showResults && !loading && filtered.length === 0 && (
          <div style={{padding: "48px 16px", textAlign: "center"}}>
            ...
            <div style={{fontSize: 15, fontWeight: 600, color: "#1A3530", marginBottom: 6}}>
              Keine Ergebnisse
            </div>
            <div style={{fontSize: 13, color: "rgba(26,53,48,0.45)"}}>
              Für „{debouncedQ}" wurde nichts gefunden.
            </div>
          </div>
        )}
            ...
            {filtered.map((item, idx) => (
              ...
              >
                ...
                <div style={{
                  ...
                  borderRadius: item.type === "profile" ? "50%" : 10,
                  overflow: "hidden",
                ...
                <div style={{flex: 1, minWidth: 0}}>
                  <div style={{
                    fontSize: 14, fontWeight: 600, color: "#1A3530",
                    overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis",
                  }}>{item.title || "—"}</div>
                  {item.sub && (
                    <div style={{
                      fontSize: 12, color: "rgba(26,53,48,0.48)",
                      overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis",
                      marginTop: 1,
                    }}>{item.sub}</div>
                  )}
                  ...
                  <div style={{
                    ...
                    color: "rgba(14,196,184,0.9)",
                  }}>
                    {item.type === "profile" ? "👤 Person"
                      : item.type === "work" ? "🎨 Werk"
                      : item.type === "experience" ? "📅 Erlebnis"
                      : item.type === "moment" ? "📝 Beitrag"
                      : "✨"}
                  </div>
                </div>

                {/* Chevron */}
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{flexShrink:0,opacity:.25}}>
                  <path d="M9 18L15 12L9 6" stroke="#1A3530" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
            ))}
          </div>

...
export function MeineInhaltePage({ onBack, userId }) {
  ...
  const [resubDlg, setResubDlg] = React.useState(null); // { type, id, title }
  const [toast,   setToast]   = React.useState(null);

  // Supabase import via window (bereits im Bundle)
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      // Nutze globalen supabase-Client aus dem Bundle
  ...
  async function resubmit(type, id) {
    ...
    const {error} = await supabase.from(table)
      .update({status:"pending_review", rejection_reason:null, rejected_at:null,
    ...
    if (error) { showToast("Fehler: "+error.message, false); }
      ...
      setter(p=>p.map(x=>x.id===id?{...x,status:"pending_review",rejection_reason:null}:x));
    }
    setResubDlg(null);
  }
    ...
    <SubPageShell title="Werke & Inhalte" emoji="🎨" onBack={onBack}>
      ...
      {resubDlg && (
        <div style={{position:"fixed",inset:0,zIndex:9900,
          ...
          <div style={{background:"#fff",borderRadius:20,padding:24,
            ...
            <div style={{fontSize:16,fontWeight:800,marginBottom:8}}>🔄 Erneut einreichen?</div>
            <div style={{fontSize:13,color:"#555",marginBottom:20}}>
              „{resubDlg.title}" wird erneut zur Prüfung eingereicht.
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setResubDlg(null)}
                style={{flex:1,padding:"11px",borderRadius:12,border:"1px solid #ddd",
                  background:"none",fontSize:13,cursor:"pointer"}}>Abbrechen</button>
              <button onClick={()=>resubmit(resubDlg.type,resubDlg.id)}
                style={{flex:2,padding:"11px",borderRadius:12,border:"none",
                  background:"#0EC4B8",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>
                ✅ Einreichen
              </button>
            </div>
          </div>
      ...
      {loading ? (
        <div style={{textAlign:"center",padding:48,color:"#888"}}>
          <div style={{fontSize:28,marginBottom:8}}>🎨</div>Lade Inhalte…
        </div>
      ) : allItems.length === 0 ? (
        <div style={{textAlign:"center",padding:48,color:"#888",fontSize:14}}>
          <div style={{fontSize:36,marginBottom:12}}>✦</div>
          Du hast noch keine Werke oder Erlebnisse eingereicht.
        </div>
      ) : allItems.map(item => {
        ...
        return (
          <div key={item.id} style={{
            ...
            <div style={{display:"flex"}}>
              {/* Thumb */}
              <div style={{width:76,flexShrink:0,minHeight:76,
              ...
              <div style={{flex:1,padding:"10px 12px"}}>
                ...
                <div style={{display:"flex",alignItems:"flex-start",
                  ...
                  <div style={{fontSize:13.5,fontWeight:700,color:"#1A1A18",lineHeight:1.3}}>
                    {item.title||"Kein Titel"}
                  </div>
                  <span style={{flexShrink:0,padding:"2px 8px",borderRadius:99,
                    background:st.bg,color:st.color,fontSize:9.5,fontWeight:700}}>
                    {st.icon} {st.label}
                  </span>
                </div>
                ...
                {isRejected && item.rejection_reason && (
                  <div style={{fontSize:11,color:"#EF4444",
                    ...
                    borderRadius:8,padding:"6px 9px",marginBottom:8,lineHeight:1.5}}>
                ...
                {item.admin_comment && (
                  <div style={{fontSize:11,color:"#555",
                    ...
                    borderRadius:8,padding:"6px 9px",marginBottom:8,lineHeight:1.5}}>
                    <b>Hinweis:</b> {item.admin_comment}
                  </div>
                )}
                ...
                {isRejected && (
                  <button
                    onClick={()=>setResubDlg({type:item._type,id:item.id,title:item.title})}

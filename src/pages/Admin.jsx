...
// ── Notification senden (wiederverwendbar) ──────────────────
...
// ── Content-Prüfung Tab ──────────────────────────────────────
function ContentTab({ onCountChange }) {
  ...
  const load = useCallback(async () => {
    ...
    const [wRes, eRes] = await Promise.all([
      supabase.from("works")
        .select("id,user_id,creator_id,title,description,cover_url,category,status,created_at,admin_comment,rejection_reason,profiles(display_name,username,avatar_url)")
        .eq("status", "pending_review")
        .order("created_at", { ascending: false })
        .limit(100),
      supabase.from("experiences")
        .select("id,user_id,title,description,cover_url,category,experience_type,status,created_at,admin_comment,rejection_reason,profiles(display_name,username,avatar_url)")
        .eq("status", "pending_review")
        .order("created_at", { ascending: false })
        .limit(100),
    ]);
    ...
    setWorks(w);
    setExps(e);
    onCountChange?.(w.length + e.length);
    setLoading(false);
  }, [onCountChange]);
  ...
  async function approve(type, id, userId, itemTitle) {
    ...
    if (error) {
      ...
      // Notification an Talent
      await sendNotification({
        userId: uid,
        type:   "content_approved",
        title:  `Dein ${type === "werk" ? "Werk" : "Erlebnis"} wurde freigegeben ✅`,
        body:   `"${itemTitle}" ist jetzt öffentlich sichtbar und im Feed.`,
        actionUrl: type === "werk" ? "/mein-hui" : "/mein-hui",
  ...
  async function rejectConfirm() {
    ...
    if (error) {
      ...
      await sendNotification({
        ...
        title:  `Dein ${type === "werk" ? "Werk" : "Erlebnis"} wurde abgelehnt`,
        body:   reason
          ? `"${title}" konnte nicht freigegeben werden.`
          : `"${title}" konnte nicht freigegeben werden. Bitte überarbeite deinen Inhalt.`,
        actionUrl: "/mein-hui",
        metadata: {
          content_type:     type,
          content_id:       id,
      ...
      showToast("❌ Abgelehnt & Talent benachrichtigt");
      if (type === "werk") setWorks(p => p.filter(x => x.id !== id));
      else                 setExps(p  => p.filter(x => x.id !== id));
      onCountChange?.(prev => Math.max(0, (prev || 1) - 1));
    }
  ...
  return (
    <div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{fontSize:13, fontWeight:600, color:C.sub, marginBottom:6}}>
              Ablehnungsgrund (sichtbar für das Talent):
            </div>
            <textarea
              value={rejectReason}
              onChange={e => setRejectReason(e.target.value)}
              placeholder="z.B. Bilder zu unscharf, Beschreibung fehlt, inhaltlich nicht passend…"
              rows={4}
              style={{
            ...
            <div style={{display:"flex", gap:10}}>
              <button onClick={() => { setRejectDlg(null); setRejectReason(""); }} style={{
                flex:1, padding:"12px", borderRadius:12, border:`1px solid ${C.border}`,
                background:"none", color:C.sub, fontSize:13, fontWeight:600,
                cursor:"pointer",
              }}>Abbrechen</button>
              <button onClick={rejectConfirm} style={{
                flex:2, padding:"12px", borderRadius:12, border:"none",
                background:C.red, color:"white", fontSize:13, fontWeight:700,
                cursor:"pointer",
              }}>✕ Ablehnen & benachrichtigen</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{...card, display:"flex", alignItems:"center",
        ...
        <div>
          <div style={{fontSize:18, fontWeight:800, color:C.text}}>📋 Content-Prüfung</div>
      ...
      {items.length === 0 ? (
        ...
        return (
          <div key={item.id} style={{...card, padding:0, overflow:"hidden", marginBottom:12}}>
            <div style={{display:"flex"}}>
              {/* Cover */}
              <div style={{
                width:88, flexShrink:0, minHeight:88,
                ...
                <div style={{
                  ...
                  background: isWerk ? "rgba(245,166,35,0.92)" : "rgba(42,191,172,0.92)",
              ...
              <div style={{flex:1, padding:"12px 14px"}}>
                ...
                {(item.category || item.experience_type) && (
                  <div style={{
                    ...
                    background:C.card2, border:`1px solid ${C.border}`,
                    fontSize:10.5, color:C.sub, marginBottom:10,
                  }}>
                    {item.category || item.experience_type}
                  </div>
                )}
                ...
                <div style={{display:"flex", gap:8}}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setRejectReason("");
                      setRejectDlg({ type: item._type, id: item.id, userId: uid, title: item.title });
                    }}
                    style={{
...
export default function Admin() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setPending((wPendingRes.count || 0) + (ePendingRes.count || 0));
      setLoading(false);
    }
    load();
  }, []);
  ...
  return (
    <div style={s}>
      ...
      {/* FREIGABEN */}
      {tab === "content" && (
        <ContentTab onCountChange={setPending}/>
      )}

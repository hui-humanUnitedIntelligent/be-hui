  ...
  const content = (
    <>
      ...
      {showInfoModal && ReactDOM.createPortal(
        <ProjectInfoModal app={app} onClose={() => setShowInfoModal(false)} />,
        document.body
      )}
...
// ════════════════════════════════════════════════════════════════
// PROJEKT-INFO-MODAL — Vollständige Projektinformationen
// ════════════════════════════════════════════════════════════════
function ProjectInfoModal({ app, onClose }) {
  const a = app;
  ...
  const images    = mediaList.filter(u => isImage(u));
  const videos    = mediaList.filter(u => isVideo(u));
  const pdfs      = mediaList.filter(u => isPdf(u));
  const others    = mediaList.filter(u => !isImage(u) && !isVideo(u) && !isPdf(u));

  // Social-Links
  const socialLinks = [
    a.website   && { label: "Website",   icon: "🌐", url: a.website },
    a.instagram && { label: "Instagram", icon: "📸", url: a.instagram.startsWith("http") ? a.instagram : `https://instagram.com/${a.instagram.replace("@","")}` },
    a.linkedin  && { label: "LinkedIn",  icon: "💼", url: a.linkedin },
    a.youtube   && { label: "YouTube",   icon: "▶️", url: a.youtube },
    a.other_links && { label: "Link",    icon: "🔗", url: a.other_links },
  ].filter(Boolean);

  const fmtEur = (n) => n ? `${Number(n).toLocaleString("de-DE")} €` : null;
  const goalStr = fmtEur(a.funding_goal);

  // Info-Zeile: Icon + Label + Wert
  ...
  const SectionTitle = ({ children }) => (
    <div style={{ fontSize:10, fontWeight:800, color:"#0DC4B5", letterSpacing:"0.09em",
      textTransform:"uppercase", marginBottom:10, marginTop:20,
      paddingBottom:6, borderBottom:"1px solid rgba(13,196,181,0.2)" }}>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:10600,
      background:"#F7F7F5", overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:3,
        ...
        <div style={{ flex:1, fontSize:16, fontWeight:900, color:"#141422",
          letterSpacing:"-0.02em", overflow:"hidden",
          textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{a.project_name}</div>
        <button onClick={onClose} style={{
          width:32, height:32, borderRadius:"50%", border:"none",
          background:"rgba(0,0,0,0.1)", fontSize:16, cursor:"pointer",
          display:"flex", alignItems:"center", justifyContent:"center",
          color:"#555", flexShrink:0 }}>✕</button>
      </div>

      <div style={{ padding:"0 16px calc(80px + env(safe-area-inset-bottom,0px))" }}>

        {/* ── Cover — kompakt, max 220px ── */}
        {a.cover_url && (
          <div style={{ margin:"0 -16px 0", position:"relative" }}>
            <img src={a.cover_url} alt={a.project_name}
              style={{ width:"100%", maxHeight:220, objectFit:"cover", display:"block" }}
              onError={e => { e.target.style.display="none"; }} />
            {/* Gradient-Overlay unten */}
            <div style={{ position:"absolute", bottom:0, left:0, right:0, height:60,
              background:"linear-gradient(to top, rgba(247,247,245,1), transparent)" }}/>
          </div>
        )}
        ...
        {(a.contact_name || a.applicant_name) && (
          <div style={{ display:"inline-flex", alignItems:"center", gap:6,
            background:"rgba(13,196,181,0.1)", borderRadius:99,
            padding:"5px 14px", marginTop:14, marginBottom:4,
        ...
        {/* ── Kurzbeschreibung ── */}
        {a.short_desc && (
          <p style={{ margin:"14px 0 0", fontSize:14, color:"#444",
            lineHeight:1.75, fontStyle:"italic",
            borderLeft:"3px solid #0DC4B5", paddingLeft:12,
            background:"rgba(13,196,181,0.04)", borderRadius:"0 8px 8px 0",
            padding:"10px 12px" }}>
            „{a.short_desc}"
          </p>
        )}
        ...
        <div style={{ marginTop:16,
          ...
          padding:"0 12px", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
          {a.location  && <InfoRow icon="📍" label="Standort"          value={a.location} />}
          {a.impact_category && a.impact_category !== a.short_desc &&
                              <InfoRow icon="🏷️" label="Kategorie"      value={a.impact_category} />}
          {goalStr           && <InfoRow icon="🎯" label="Förderziel"    value={goalStr} />}
          {a.contact_name    && <InfoRow icon="👤" label="Initiator"     value={a.contact_name} />}
          {a.contact_email   && (
            <div style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"9px 0",
              borderBottom:"1px solid rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:10, color:"#0DC4B5", fontWeight:800,
                  textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:2 }}>E-Mail</div>
        ...
        {a.problem && (
          <>
            ...
            <p style={{ margin:0, fontSize:13, color:"#333", lineHeight:1.8,
              whiteSpace:"pre-wrap" }}>{a.problem}</p>
          </>
        )}
        ...
        {a.vision && (
          <>
            <SectionTitle>Vision & Umsetzung</SectionTitle>
            <p style={{ margin:0, fontSize:13, color:"#333", lineHeight:1.8,
              whiteSpace:"pre-wrap" }}>{a.vision}</p>
          </>
        )}

        {/* ── Mittelverwendung (wenn abweichend von Vision) ── */}
        {a.funding_use && a.funding_use !== a.vision && (
          <>
            <SectionTitle>Wofür wird das Geld verwendet?</SectionTitle>
            <p style={{ margin:0, fontSize:13, color:"#333", lineHeight:1.8,
              whiteSpace:"pre-wrap" }}>{a.funding_use}</p>
          </>
        )}

        {/* ── Bilder — 2-spaltiges Grid ── */}
        {images.length > 0 && (
          <>
        ...
        {(pdfs.length > 0 || others.length > 0) && (
          <>
            <SectionTitle>Dokumente & Links</SectionTitle>
            {[...pdfs, ...others].map((url, i) => (
              <a key={i} href={url} target="_blank" rel="noreferrer"
                style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px",
                  background:"rgba(13,196,181,0.07)",
                  border:"1px solid rgba(13,196,181,0.2)",
                  borderRadius:10, marginBottom:8,
                  color:"#0DC4B5", fontSize:13, fontWeight:600,
                  textDecoration:"none" }}>
                {isPdf(url) ? "📄" : "🔗"} {isPdf(url) ? "PDF ansehen" : "Link öffnen"}
              </a>
            ))}
          </>
        )}
        ...
        {socialLinks.length > 0 && (
          <>
            <SectionTitle>Online-Präsenz</SectionTitle>
        ...
        {/* ── Footer ── */}
        <div style={{ textAlign:"center", padding:"24px 0 8px", marginTop:16,
          borderTop:"1px solid rgba(13,196,181,0.15)" }}>
          <div style={{ fontSize:11, color:"#bbb", lineHeight:1.65 }}>
            Nominiert von der HUI-Community · Unterstützt durch den monatlichen Impact Pool
          </div>
        </div>
      ...
      <div style={{
        ...
        <button onClick={onClose} style={{
          ...
          cursor:"pointer", boxShadow:"0 4px 14px rgba(13,196,181,0.3)" }}>
          Schließen
        </button>
      </div>
    </div>
...
function ImpactPageInner({ currentUser: currentUserProp }) {
  ...
  return (
    <div data-impact-page style={{ width:"100%", background:T.page,
      ...
      {/* ── Orb-Clearance-Spacer — letzter Scroll-Inhalt im Impact-Tab ── */}

...
import {
  ...
  selectDiscoverBg,
  selectDiscoverMotionScale,
  selectRevealDelay,
  selectPrimaryDistrict,
  selectWanderingPath,
  selectWorldWhisper,
...
export default function DiscoverPage({ onMap, onView, onBook, refreshSignal }) {
  ...
  // ── Discover World — derived from feed signals
  const discoverWorld = useMemo(() => {
    const allItems = [...works, ...experiences].map(item => ({
      ...item,
      type: item.type || (experiences.includes(item) ? "experience" : "work_upload"),
    }));
    return mockDiscoverWorldFromFeed(allItems, null);
  }, [works, experiences]);

  const dAmb    = selectDiscoverAmbient(discoverWorld);
  const dAccent = selectDistrictAccent(discoverWorld);
  const dBg     = selectDiscoverBg(discoverWorld);
  const dMotion = selectDiscoverMotionScale(discoverWorld);
  ...
  return (
    <div style={{
      minHeight:"100vh",
      background: dBg || "rgb(254,252,250)",
      fontFamily:"-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Segoe UI', sans-serif",
      paddingBottom: 100,
    }}>
      <style>{CSS}</style>

      {/* ── World whisper — very rare atmospheric text, not a notification */}
      {dWhisper && (
        <div style={{
          ...
          borderRadius:12,
          border:`1px solid ${dAccent}22`,
          fontSize:11.5, color:dAccent, fontWeight:500,
          letterSpacing:"0.01em", opacity:0.90,
        ...
        {showSearch ? (
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              ...
              {dPath[0] && (
                <span style={{
                  fontSize:11, color: dAccent, fontWeight:600, letterSpacing:"0.04em",
                  opacity:0.85, display:"block", marginTop:1,
      ...
      >
        {PILLS.map((pill, i) => {
          ...
          const distId   = pillToDistrict(pill.id);
          // Pills glow in district's accent color when that district is primary
          const isPrimary= distId && selectPrimaryDistrict(discoverWorld) === distId;
          const glowColor= isPrimary ? dAccent : null;
          ...
          return (
            <button
              ...
              style={{
                background: active
                  ? (glowColor
                      ? `linear-gradient(135deg, ${glowColor} 0%, ${C.coral} 100%)`
                      : `linear-gradient(135deg, ${C.teal} 0%, ${C.coral} 100%)`)
                  : C.card,
                color: active ? "#fff" : C.ink2,
                boxShadow: active
                  ? `0 4px 16px ${glowColor ? glowColor + "55" : "rgba(22,215,197,0.30)"}`
                  : glowColor
                    ? `0 0 0 1.5px ${glowColor}44`
                    : "0 1px 6px rgba(0,0,0,0.07)",
                animation: `pillIn 0.3s ${i*0.04}s both`,
                fontWeight: active ? 700 : 500,
                transform: active ? "scale(1.04)" : "scale(1)",
                transition:"box-shadow 0.8s ease, background 0.5s ease",
              }}
            >
              {pill.label}
            </button>
      ...
      <div style={{
        ...
        <SectionHeader
          title={dPath[0]
            ? `${dPath[0].district.invitation} ✦`
            : "Heute könnte dich das berühren ✦"}
          subtitle={dPath[0]?.district?.invitationSub || ""}
        />
        <div
          className="dp-scroll"
          style={{

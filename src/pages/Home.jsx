...
function DiscoverFeed({ onView, onMatch }) {
  const cats = [
    ...
    {icon:"🎨", label:"Kunst",       bg:C.coralPale,  color:C.coral},
    {icon:"📷", label:"Fotografie",  bg:"#EDE9FE",    color:"#7C3AED"},
    {icon:"💬", label:"Coaching",    bg:"#FFFBEB",    color:C.gold},
    {icon:"🌿", label:"Natur",       bg:"#D1FAE5",    color:C.green},
    {icon:"🎵", label:"Musik",       bg:"#FCE7F3",    color:"#DB2777"},
    {icon:"🏥", label:"Gesundheit",  bg:"#D1FAE5",    color:C.green},
    {icon:"✨", label:"Events",      bg:"#FFFBEB",    color:C.gold},
  ];
  ...
  return (
    <div style={{ paddingBottom:110 }}>
        ...
        <div style={{ position:"relative", marginBottom:20 }}>
          ...
          <input
            style={{ width:"100%", background:C.card,
              border:"1.5px solid rgba(0,0,0,0.06)", borderRadius:999,
              padding:"13px 18px 13px 44px", fontSize:14, color:C.ink,
              outline:"none", fontFamily:"inherit", boxSizing:"border-box",
              boxShadow:"0 2px 10px rgba(0,0,0,0.05)" }}
            placeholder="Suche nach Menschen, Werken, Erlebnissen…"
            readOnly onFocus={onMatch}/>
        </div>
      </div>

      {/* Hero */}
      <div style={{ margin:"0 20px 24px", borderRadius:24,
        overflow:"hidden", height:195, position:"relative",
        boxShadow:"0 6px 28px rgba(0,0,0,0.14)" }}>
        <img src="https://images.unsplash.com/photo-1439066615861-d1af74d74000?w=800&q=90"
          alt="Nähe"
          style={{ width:"100%", height:"100%", objectFit:"cover",
            filter:"brightness(0.72) saturate(1.1)" }}/>
        <div style={{ position:"absolute", inset:0,
          background:"linear-gradient(to bottom,transparent 20%,rgba(0,0,0,0.70) 100%)"}}/>
      ...
      {/* Für dich empfohlen */}
      <div style={{ display:"flex", justifyContent:"space-between",
        alignItems:"center", padding:"0 20px 14px" }}>
        <div style={{ fontWeight:800, fontSize:17, color:C.ink }}>
          Für dich empfohlen
        </div>
        <button style={{ background:"none", border:"none", cursor:"pointer",
          fontSize:12, fontWeight:600, color:C.teal }}>Alle →</button>
      </div>

      <div style={{ padding:"0 20px", display:"flex",
        flexDirection:"column", gap:14 }}>
        {EXPERIENCES.map((ev,i)=>(
          <div key={i} style={{ background:C.card, borderRadius:22,
            overflow:"hidden",
            ...
            <div style={{ padding:"14px 16px" }}>
              <div style={{ fontWeight:800, fontSize:15,
                color:C.ink, marginBottom:3 }}>{ev.title}</div>
              <div style={{ fontSize:12, color:C.muted }}>{ev.sub}</div>
            </div>
          </div>
        ))}
      </div>

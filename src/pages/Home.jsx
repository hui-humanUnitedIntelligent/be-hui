...
import WirkerVerifizierungDashboard from "@/components/WirkerVerifizierungDashboard";
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3 }}>
              ...
              {Array(daysInMonth).fill(null).map((_, i) => {
                ...
                return (
                  <button key={d} onClick={() => handleDayClick(d)} style={{ aspectRatio: "1", borderRadius: "50%", border: isToday && !isSelected ? `2px solid ${CORAL}55` : "2px solid transparent", cursor: avail && !past ? "pointer" : "default", background: isSelected ? `linear-gradient(135deg, ${CORAL}, ${GOLD})` : avail && !past ? `${TEAL}15` : "transparent", color: isSelected ? "white" : past ? "#ddd" : avail ? TEAL : "#ccc", fontWeight: avail && !past ? 800 : 400, fontSize: 14, position: "relative", transition: "all 0.15s", boxShadow: isSelected ? `0 3px 12px ${CORAL}44` : "none" }}>
                    {d}
      ...
      <div style={{ padding: "14px 18px 28px", borderTop: "1px solid #f5f5f3", background: "white" }}>
        ...
        {step === 5 && (
          <div>
            ...
            <style>{`@keyframes toastIn { from { opacity: 0; transform: translateX(-50%) translateY(20px); } to { opacity: 1; transform: translateX(-50%) translateY(0); } } @keyframes spin { to { transform: rotate(360deg); } }`}</style>
...
// ImpactTrackerPage extracted to components/ImpactTrackerPageComponent.jsx
function ImpactTrackerPage({ onClose }) {
    return null; // ImpactTrackerPage extracted
}
...
function AppHeader({ cartCount, onCartClick, onNotifClick, notifCount, onVerifizierungClick }) {
  return (
    <div style={{ background: "white", padding: "14px 16px 10px", boxShadow: "0 1px 8px rgba(0,0,0,0.06)", position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <button onClick={onVerifizierungClick} title="Wirker-Verifizierung" style={{ background: "none", border: "none", cursor: "pointer", padding: 6, display: "flex" }}><BadgeCheck size={22} color="#2ABFAC" /></button>
...
function SearchBar({ onClick, onKarteClick, onMatchClick }) {
  return (
    <div style={{ background: "white", padding: "5px 16px 7px", position: "sticky", top: 54, zIndex: 99, borderBottom: "1px solid #f0f0f0", display: "flex", gap: 8 }}>
      <div onClick={onClick} style={{ flex: 1, background: "#f3f3f3", borderRadius: 10, padding: "7px 12px", display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
        <Search size={15} color="#aaa" />
        <span style={{ color: "#bbb", fontSize: 13, flex: 1 }}>Suche nach Talent, Werk, Name…</span>
        <div style={{ background: `${TEAL}18`, borderRadius: 7, padding: "2px 7px", display: "flex", alignItems: "center", gap: 3 }}><SlidersHorizontal size={12} color={TEAL} /><span style={{ fontSize: 10, color: TEAL, fontWeight: 700 }}>Filter</span></div>
      </div>
      <button onClick={onMatchClick} style={{ background: `linear-gradient(135deg, ${CORAL}, ${GOLD})`, border: "none", borderRadius: 10, padding: "0 11px", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", gap: 5, fontWeight: 800, fontSize: 13, color: "white", minHeight: 34, boxShadow: `0 3px 10px ${CORAL}44` }}>
        ✨
      </button>
      <button onClick={onKarteClick} style={{ background: `${TEAL}15`, border: "none", borderRadius: 10, padding: "0 11px", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", gap: 5, fontWeight: 700, fontSize: 12, color: TEAL, minHeight: 34 }}>
        🗺
      </button>
    </div>
  );
...
export default function App() {
  ...
  const [showVerifizierung, setShowVerifizierung] = useState(false);
  ...
  return (
    <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: "#f7f7f5", fontFamily: "'Inter', -apple-system, sans-serif", position: "relative" }}>
      {page === "home" && (<>
        <AppHeader cartCount={cart.length} onCartClick={() => setShowCart(true)} onNotifClick={() => setShowNotifications(true)} notifCount={notifCount} onVerifizierungClick={() => setShowVerifizierung(true)} />
      ...
      {showVerifizierung && <WirkerVerifizierungDashboard onClose={() => setShowVerifizierung(false)} onViewWirker={(w) => { setShowVerifizierung(false); viewWirker(w); }} />}
      <style>{`

...
import { compileRules } from '../../policy/ruleCompiler.js';
import { compilePolicies } from '../../policy/policyCompiler.js';
...
export function buildAuthorityState(options = {}) {
  ...
  _cache = Object.freeze({
    ...
    domainRegistry: Object.freeze(buildDomainRegistryFromContracts()),
    policyEngineRegistry: Object.freeze({
      policies: compilePolicies(),
      rules: compileRules(),
      source: 'docs/governance/domain-contracts.json',
      authority: 'ARCH-006',
    }),
...
function buildRules(constitution, goldenRules, principles, adrs, rfcs, policies) {
  ...
  // ARCH-006: Scanner-Regeln ausschließlich aus Policy Engine (Domain Contracts)
  for (const sr of compileRules()) {
    const adr = adrs.find(a => a.id === 'ADR-002');
    rules.push(createRule({
      id: sr.id,
      title: sr.description || sr.type,
      description: `Policy enforcement: ${sr.type} (${sr.severity}) — Domain ${sr.domainId}`,
      source: 'docs/governance/domain-contracts.json',
      version: 'ARCH-006',
      status: adr ? normalizeStatus(adr.status) : STATUS.ACCEPTED,
      authority: 'ARCH-006',
      validSince: adr?.date || '2026-06-30',
      scope: 'code',
      priority: RULE_PRIORITY.SCANNER,
      affectedDomains: [sr.domainId],
      metadata: { severity: sr.severity, violationType: sr.type, policyId: sr.policyId },
      derived: true,
    }));
  }
...
function buildDomainRegistryFromContracts() {
  const contracts = loadContracts();
  const registry = {};
  for (const contract of contracts.domains) {
    registry[contract.id] = Object.freeze({
      id: contract.id,
...
function buildLayerRegistry() {
  const domains = buildDomainRegistryFromContracts();
  const layers = {};
  for (const [id, domain] of Object.entries(domains)) {
    const layer = domain.layer ?? 0;
...
export {
  buildDomainRegistryFromContracts,
  CANONICAL_DOMAINS,
  SCANNER_RULES,
  ADR_METADATA,
  RFC_METADATA,

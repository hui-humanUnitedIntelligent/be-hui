...
export function buildDecisionTrace(input, scan) {
  ...
  if (violation) {
    ...
    if (violation.explanation) {
      appliedRules.push({
        id: violation.explanation.rule,
        type: violation.explanation.ruleType,
        constitution: violation.explanation.constitution,
        adr: violation.explanation.adr,
        rfc: violation.explanation.rfc,
        source: 'ARCH-006-policy-engine',
      });
      sources.push({ type: 'domain-contract', ref: 'docs/governance/domain-contracts.json', domain: violation.explanation.domain });
    } else {
    ...
    const adrs = violation.explanation?.adr?.length
      ? violation.explanation.adr.map(id => ({ id, title: id }))
      : getAdrsForViolationType(violation.type, scan.governance.adrs);
    ...
    conclusion = `${violation.file} verletzt ${violation.explanation?.ruleType || violation.type}. Empfehlung: ${title}. Betroffene Dateien: ${affected.slice(0, 5).join(', ')}. Risiko: ${riskScore}/10.`;
  }

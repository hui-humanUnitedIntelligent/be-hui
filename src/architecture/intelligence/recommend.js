...
import { getRecommendationsFromViolations } from '../policy/explanationEngine.js';
...
export function recommendForViolation(violation, scan) {
  // ARCH-006: Policy-Engine-Erklärungen bevorzugen
  if (violation.explanation) {
    return {
      id: `REC-${violation.id}`,
      violationId: violation.id,
      title: `Beheben: ${violation.type}`,
      file: violation.file,
      line: violation.line,
      steps: violation.explanation.recommendation || [],
      migration: violation.explanation.migration,
      risk: violation.explanation.risk,
      confidence: violation.explanation.confidence,
      domain: violation.explanation.domain,
      contract: violation.explanation.contract,
  ...
  const rule = violation.explanation?.rule || { title: violation.type };

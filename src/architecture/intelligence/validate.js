...
import { getDomainForPath, isImportAllowed } from '../policy/domainResolver.js';
import { getContract } from '../policy/contractLoader.js';
...
export function validateProposal(proposal, scan) {
  ...
  if (proposal.addDbWrite) {
    ...
    let coreTables = new Set();
    try {
      const wirkung = getContract('WIRKUNG');
      coreTables = new Set(wirkung.data?.owned || []);
    } catch { /* fallback empty */ }

    if (coreTables.has(table) && !file.includes('src/core/') && !file.includes('src/services/')) {
      violations.push({
        type: 'CORE_BYPASS',

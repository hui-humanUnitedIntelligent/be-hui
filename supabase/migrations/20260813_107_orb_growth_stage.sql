-- ORB-GROWTH-STAGE (2026-08-13)
-- Additiv, keine Aenderung bestehender Tabellen/Logik (No-Regression-Protection).
-- Berechnet Wachstumsstufe (1-6) fuer den Orb-Button.
-- SECURITY DEFINER, da RLS-Tabellen aggregiert werden.
...
BEGIN
  ...
  SELECT
    ...
    + (SELECT COUNT(*) FROM order_items oi
         ...
         WHERE o.customer_id = p_user_id AND o.state <> 'aborted')
    + (SELECT COUNT(*) FROM order_items WHERE seller_id = p_user_id)
    + (SELECT COUNT(*) FROM talent_bookings
         WHERE customer_id = p_user_id AND status <> 'pending_payment')
    + (SELECT COUNT(*) FROM talent_bookings
         WHERE seller_id = p_user_id AND status <> 'pending_payment')

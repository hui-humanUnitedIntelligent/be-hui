-- ═══════════════════════════════════════════════════════════════════════
-- HUI 024 — MASTER PRODUCTION SCHEMA (FIXED v2)
-- Datum: 2026-05-15
--
-- FIX: Pre-Flight Block entfernt alle alten FK-Constraints die auf
--      wirker_profiles(wirker_id) referenzieren — diese Spalte existiert
--      nicht. wirker_profiles nutzt 'id' als PK und 'user_id' als FK.
--
-- GARANTIEN:
...
-- ─────────────────────────────────────────────────────────────────────
-- PRE-FLIGHT: Alle alten kaputten FK-Constraints entfernen
-- Supabase kann keine ALTER TABLE ausführen wenn veraltete
-- Constraints auf nicht-existierende Spalten zeigen.
-- ─────────────────────────────────────────────────────────────────────
DO $$
DECLARE
  r RECORD;
BEGIN
  -- Entferne ALLE Foreign Key Constraints die auf wirker_profiles zeigen
  -- (egal auf welche Spalte — wir setzen sie danach korrekt neu)
  FOR r IN
  ...
  -- Entferne auch alle Constraints die wirker_id als Spaltenname nutzen
  -- und auf eine nicht-existierende Spalte in wirker_profiles zeigen
  FOR r IN
  ...
  LOOP
    ...
    RAISE NOTICE 'Dropped wirker_id FK: %.%', r.table_name, r.constraint_name;
  END LOOP;
END $$;
...
-- location (text oder GEOGRAPHY) → location_text migrieren
DO $$ BEGIN
  IF EXISTS (
    ...
    SET location_text = ST_AsText(location::geometry)
...
-- follower_count / followers_count synchron halten
...
-- PK = id (uuid), FK auf auth.users = user_id (UNIQUE)
-- KEINE Spalte namens 'wirker_id' — alle alten Referenzen wurden
-- im Pre-Flight Block oben bereits entfernt.
-- ─────────────────────────────────────────────────────────────────────
...
-- RECOMMENDATIONS
-- wirker_id → REFERENCES auth.users(id)  (NOT wirker_profiles)
...
-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 7: BOOKING & PAYMENT TABLES
-- wirker_id in bookings → REFERENCES auth.users(id)  (NOT wirker_profiles)
-- ─────────────────────────────────────────────────────────────────────
...
CREATE POLICY payout_select_own ON public.payouts
  FOR SELECT TO authenticated USING (auth.uid() = wirker_id);

CREATE INDEX IF NOT EXISTS payout_wirker_id_idx ON public.payouts(wirker_id);

-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 8: MESSAGING TABLES
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 9: NOTIFICATIONS & SETTINGS
-- ─────────────────────────────────────────────────────────────────────
...
CREATE POLICY ps_select_own ON public.privacy_settings
  FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY ps_upsert_own ON public.privacy_settings
  FOR ALL TO authenticated USING (auth.uid() = user_id);

-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 10: IMPACT TABLES
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 11: AVAILABILITY & MEDIA
-- wirker_id in availability_slots → REFERENCES auth.users(id)
-- ─────────────────────────────────────────────────────────────────────

-- AVAILABILITY_SLOTS
-- wirker_id = auth.users.id des Wirkers (NOT wirker_profiles.id)
...
CREATE POLICY avail_insert_own ON public.availability_slots
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = wirker_id);
CREATE POLICY avail_update_own ON public.availability_slots
  FOR UPDATE TO authenticated USING (auth.uid() = wirker_id);
  ...
  FOREACH b IN ARRAY ARRAY['media','stories','works','experiences','avatars'] LOOP
    ...
    EXECUTE format(
      'CREATE POLICY hui_stor_upload_%s ON storage.objects
       FOR INSERT TO authenticated WITH CHECK (bucket_id = %L)', b, b);
    EXECUTE format(
...
-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 13: DATA RECOVERY
-- ─────────────────────────────────────────────────────────────────────
UPDATE public.works SET status = 'published'
WHERE (status IS NULL OR status = '')
  AND (state IS NULL OR state IN ('published','active'));
...
-- ─────────────────────────────────────────────────────────────────────
-- SCHRITT 14: SCHEMA-CACHE FLUSH + VERIFIKATION
-- ─────────────────────────────────────────────────────────────────────
NOTIFY pgrst, 'reload schema';

SELECT
  '✓ HUI 024 FIXED — Master Schema komplett' AS status,
  NOW()                                       AS completed_at,
  ...
  (SELECT COUNT(*) FROM public.stories  WHERE status = 'published') AS stories_pub,

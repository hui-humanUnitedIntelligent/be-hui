-- ═══════════════════════════════════════════════════════════════════════
-- HUI 024 — MASTER PRODUCTION SCHEMA  (FIXED v3)
-- Datum: 2026-05-15
--
-- ARCHITEKTUR-REGEL DIESES SCRIPTS:
--   Jede Tabelle die bereits in Supabase existieren kann wird NUR via
--   ALTER TABLE verwaltet — NIE via CREATE TABLE allein.
--   Das Schema:
--     1. CREATE TABLE IF NOT EXISTS  (nur Kern-Spalten)
--     2. ALTER TABLE ADD COLUMN IF NOT EXISTS  (alle weiteren Spalten)
--   Dadurch existieren ALLE Spalten garantiert — egal ob Tabelle neu ist.
--
-- KRITISCHER FIX gegenüber v1/v2:
--   wirker_id in bookings/payouts/availability_slots wird IMMER via
--   ALTER TABLE ADD COLUMN IF NOT EXISTS gesetzt.
--   Die CREATE TABLE-Blöcke definieren KEINE wirker_id mehr — sie kommen
--   ausschliesslich aus den ALTER TABLE-Blöcken.
--   So existiert wirker_id garantiert wenn Policies sie referenzieren.
--
-- GARANTIEN:
--   ✓ 100% idempotent — mehrfach ausführen ist sicher
--   ✓ Keine bestehenden Daten werden gelöscht
--   ✓ wirker_id existiert garantiert in allen Tabellen die sie brauchen
--   ✓ Keine REFERENCES auf wirker_profiles(irgendwas)
...
-- ─────────────────────────────────────────────────────────────────────
-- PRE-FLIGHT A: Kaputte FK-Constraints entfernen
-- Entfernt alle Foreign Keys die auf nicht-existente Spalten zeigen
-- ─────────────────────────────────────────────────────────────────────
DO $$
DECLARE r RECORD;
BEGIN
  -- Alle FKs die auf wirker_profiles zeigen droppen
  -- (wird danach korrekt neu gesetzt — falls überhaupt nötig)
  FOR r IN
    ...
    WHERE tc.constraint_type = 'FOREIGN KEY'
      AND tc2.table_name = 'wirker_profiles'
      AND tc.table_schema = 'public'
  LOOP
    EXECUTE format('ALTER TABLE public.%I DROP CONSTRAINT IF EXISTS %I',
      r.table_name, r.constraint_name);
    RAISE NOTICE 'Dropped FK → wirker_profiles: %.%', r.table_name, r.constraint_name;
  END LOOP;
END $$;

-- ─────────────────────────────────────────────────────────────────────
-- PRE-FLIGHT B: Kaputte RLS Policies entfernen die auf nicht-existente
-- Spalten referenzieren könnten
-- ─────────────────────────────────────────────────────────────────────
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
    ...
    WHERE schemaname = 'public'
      AND (
        policyname LIKE '%wirker%' OR
        policyname LIKE '%book%'   OR
        policyname LIKE '%avail%'  OR
        policyname LIKE '%payout%' OR
        policyname LIKE '%rec%'
      )
  LOOP
...
-- location (GEOGRAPHY oder text) → location_text migrieren
DO $$ BEGIN
...
-- ═══════════════════════════════════════════════════════════════════════
-- 5. WIRKER_PROFILES
-- PK = id  |  FK = user_id (UNIQUE) → auth.users(id)
-- Es gibt KEINE Spalte namens "wirker_id" in dieser Tabelle.
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
-- 13. RECOMMENDATIONS
-- reviewer_id und wirker_user_id → auth.users(id)
-- Spaltenname: wirker_user_id  (kein "wirker_id" um Konflikte zu vermeiden)
-- ═══════════════════════════════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS public.recommendations (
  id             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  wirker_user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- Falls Tabelle bereits mit wirker_id existiert: Spalte umbenennen
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='recommendations'
    AND column_name='wirker_id')
  AND NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='recommendations'
    AND column_name='wirker_user_id')
  THEN
    ALTER TABLE public.recommendations RENAME COLUMN wirker_id TO wirker_user_id;
    RAISE NOTICE 'recommendations.wirker_id → wirker_user_id umbenannt';
  END IF;
END $$;

-- Fallback: wirker_user_id hinzufügen falls beides fehlt
ALTER TABLE public.recommendations
  ADD COLUMN IF NOT EXISTS wirker_user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE;
...
CREATE INDEX IF NOT EXISTS rec_wirker_user_id_idx ON public.recommendations(wirker_user_id);

-- ═══════════════════════════════════════════════════════════════════════
-- 14. BOOKINGS
-- booker_id = User der bucht → auth.users(id)
-- provider_id = Wirker der gebucht wird → auth.users(id)
-- Spaltenname provider_id statt wirker_id — vermeidet alle Konflikte
-- ═══════════════════════════════════════════════════════════════════════
...
-- Falls Tabelle bereits mit user_id/wirker_id existiert: Spalten sichern
DO $$ BEGIN
  -- user_id → booker_id
  IF EXISTS (SELECT 1 FROM information_schema.columns
  ...
  THEN
    ALTER TABLE public.bookings RENAME COLUMN user_id TO booker_id;
    RAISE NOTICE 'bookings.user_id → booker_id';
  END IF;
END $$;

DO $$ BEGIN
  -- wirker_id → provider_id
  IF EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='bookings' AND column_name='wirker_id')
  AND NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='bookings' AND column_name='provider_id')
  THEN
    ALTER TABLE public.bookings RENAME COLUMN wirker_id TO provider_id;
    RAISE NOTICE 'bookings.wirker_id → provider_id';
  END IF;
END $$;

-- Fallback: Spalten hinzufügen falls fehlend
...
-- ═══════════════════════════════════════════════════════════════════════
-- 16. PAYOUTS
-- Spaltenname: provider_id  (statt wirker_id)
-- ═══════════════════════════════════════════════════════════════════════
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='payouts' AND column_name='wirker_id')
  AND NOT EXISTS (SELECT 1 FROM information_schema.columns
    WHERE table_schema='public' AND table_name='payouts' AND column_name='provider_id')
  THEN
    ALTER TABLE public.payouts RENAME COLUMN wirker_id TO provider_id;
    RAISE NOTICE 'payouts.wirker_id → provider_id';
  END IF;
END $$;
...
DO $$ BEGIN
  ...
  THEN
    ALTER TABLE public.availability_slots RENAME COLUMN wirker_id TO provider_id;
    RAISE NOTICE 'availability_slots.wirker_id → provider_id';
  END IF;
END $$;
...
BEGIN
  FOREACH b IN ARRAY ARRAY['media','stories','works','experiences','avatars'] LOOP
    ...
    EXECUTE format(
      'CREATE POLICY hui_stor_upload_%s ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = %L)', b, b);
    EXECUTE format(
      'CREATE POLICY hui_stor_select_%s ON storage.objects FOR SELECT USING (bucket_id = %L)', b, b);
    EXECUTE format(
      'CREATE POLICY hui_stor_delete_%s ON storage.objects FOR DELETE TO authenticated USING (bucket_id = %L AND owner = auth.uid()::text)', b, b);
  END LOOP;
END $$;

-- ═══════════════════════════════════════════════════════════════════════
-- 28. DATA RECOVERY — bestehende Rows reparieren
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
SELECT
  '✓ HUI 024 v3 — Master Schema komplett' AS status,
  NOW()                                    AS completed_at,

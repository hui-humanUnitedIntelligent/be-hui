-- Ambassador Fix 4 — Drop ALL Overloads + sauber neu deployen
-- Strategie: DROP CASCADE auf alle bekannten Signaturen

-- === rpc_get_ambassador_referrals ===
DO $$ DECLARE r RECORD;
BEGIN
  FOR r IN SELECT oid FROM pg_proc WHERE proname = 'rpc_get_ambassador_referrals' LOOP
...
DO $$ DECLARE r RECORD;
BEGIN
  FOR r IN SELECT oid FROM pg_proc WHERE proname = 'rpc_get_all_ambassadors' LOOP
    EXECUTE 'DROP FUNCTION IF EXISTS ' || r.oid::regprocedure || ' CASCADE';
  END LOOP;
END $$;
